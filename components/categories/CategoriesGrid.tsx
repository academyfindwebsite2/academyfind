...
//type SubCategory = { id: string; name: string; slug: string; children: LeafCategory[] };

interface CategoryGridProps {
  childrenCategories: SubCategory[]; // These are Level 1 categories (e.g., "Engineering")
}

export default function CategoryGrid({ childrenCategories }: CategoryGridProps) {
            ...
            <div className="space-y-2">
              {subCat.children.length > 0 ? (
                subCat.children.map((leaf) => (
                  <Link
                    key={leaf.slug}
                    href={`/${leaf.slug}`} // 👈 Navigating to the SEO page
